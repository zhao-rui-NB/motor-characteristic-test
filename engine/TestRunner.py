...
class TestRunner:
    ...
    def test_dc_resistance(self, motor:Motor):
        ...
        self.device_manager.power_supply.set_source_mode(0) # 0 AC+DC-INT # e 設定ASR6450  AC+DC+INT 三相系統
        ...
        for i in range(1 if is_single_phase else 3):
            ...
            self.device_manager.power_supply.set_voltage_offset(0.1)
            time.sleep(0.3)
            # read WT333 voltage
        ...
        for i in range(1 if is_single_phase else 3):
            ...
            while True:
                ...
                now_current = max(abs(data.get('I1')), abs(data.get('I2')))
                
                if now_current >= motor.rated_current:
                    for _ in range(5):
                        data:dict = self.device_manager.power_meter.read_data()
    ...
    def run_open_circuit_test(self, motor:Motor):
        ...
        self.device_manager.power_supply.set_current_limit(motor.no_load_current*1.2)
        # i 設定ASR6450 頻率輸出命令
        ...
        for i in range(10):
            time.sleep(1)
            data:dict = self.device_manager.power_meter.read_data()
            print(f"[run_open_circuit_test] read from power meter, V1:{data.get('V1')}, I1:{data.get('I1')}, P1:{data.get('P1')}, PF1:{data.get('PF1')}")
    ...
    def run_lock_rotor_test(self, motor:Motor):
        ...
        raw_data = []
        while True:
            self.device_manager.power_supply.set_voltage_offset(v_test)
            time.sleep(0.1)
            data:dict = self.device_manager.power_meter.read_data()
            v_str = f'V1:{data.get('V1')}, V2:{data.get('V2')}, V3:{data.get('V3')}'
            i_str = f'I1:{data.get('I1')}, I2:{data.get('I2')}, I3:{data.get('I3')}'
    ...
    def run_load_test(self, motor:Motor):
        ...
        self.device_manager.power_supply.set_voltage(5)
        # h. 設定ASR6450 最大電流命令(滿載電流*120%)
        self.device_manager.power_supply.set_current_limit(motor.rated_current*6) # 6倍滿載電流
        # i. 設定ASR6450 頻率輸出命令
        self.device_manager.power_supply.set_frequency(motor.frequency)
        # j. 設定ASR6450 電壓輸出命令
        self.device_manager.power_supply.set_output(1)
        ...
        # m. 提示 ”待測馬達請安裝於扭矩測試系統”
        # n. 等待啟動確認按鍵(提示輸出確認?)
        print('[run_load_test] ”待測馬達請安裝於扭矩測試系統”')
        input('Press Enter to continue:')

        # o. 制動控制的輸出電流=0 (無載)
        self.device_manager.plc_mechanical.set_break(0)
        
        # p. 設定ASR6450 輸出電壓=20%額定電壓(減少啟動電流衝擊) 設定外部輸出開關 (啟動測試電壓輸出Y24 OR Y25) , 逐漸調整輸出電壓到額定電壓(每0.5 Sec增加 5%)等待3 Sec 讓系統穩定後, 開是循環測試:
        if is_single_phase:
            self.device_manager.plc_electric.set_motor_output_single()
        else:
            self.device_manager.plc_electric.set_motor_output_three()
            
        for i in range(20, 100+1, 5): # 20% to 100%
